  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/50 backdrop-blur-xl border border-white/[0.08] rounded-3xl overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        {comments.map((c) => (
          <div key={c.id} className={`group relative flex gap-3 animate-fade-in ${c.isPinned ? 'bg-cyan-500/5 -mx-2 px-2 py-3 rounded-2xl border border-cyan-500/10' : ''}`}>
            <div className="h-8 w-8 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center shrink-0">
               <svg className="h-4 w-4 text-slate-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
            </div>
            <div className="min-w-0 flex-1">
               <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-xs font-bold truncate ${c.user.role === 'ADMIN' ? 'text-red-400' : c.user.role === 'STAFF' ? 'text-violet-400' : 'text-slate-300'}`}>
                    {c.user.name || c.user.email.split('@')[0]}
                  </span>
                  {c.isPinned && <span className="text-[10px] text-cyan-400 flex items-center gap-1">📌 Pinned</span>}
               </div>
            ...
            {(isAdmin() || isStaff() || c.userId === user?.id) && (
              <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                 {(isAdmin() || isStaff()) && (
                   <button onClick={() => togglePin(c.id, !c.isPinned)} className="p-1.5 text-slate-500 hover:text-cyan-400 transition-colors">
                 ...
                 <button onClick={() => deleteComment(c.id)} className="p-1.5 text-slate-500 hover:text-red-400 transition-colors">
        ...
        {user ? (
          <form onSubmit={postComment} className="space-y-2">
            <div className="relative">
              ...
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
              </button>
            </div>
          ...
          <div className="text-center py-2">
            <Link href="/account/signin" className="text-xs font-bold text-cyan-400 hover:underline">Sign in to chat</Link>
          </div>
        )}
      </div>
    </div>
