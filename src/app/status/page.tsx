  ...
  return (
    <div className="pt-24 pb-20 px-4 sm:px-6 min-h-screen">
      <div className="mx-auto max-w-3xl">
        <div className="mb-8">
          <Link href="/" className="text-sm text-slate-400 hover:text-white transition-colors mb-4 inline-block">
            ← Back to Home
          </Link>
        ...
        <div className={`rounded-2xl border p-6 mb-8 shadow-xl ${hasActiveIssues ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
          <div className="flex items-center gap-4">
            <div className={`flex h-12 w-12 items-center justify-center rounded-full ${hasActiveIssues ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
              {hasActiveIssues ? (
            ...
            <div>
              <div className={`text-lg font-bold ${hasActiveIssues ? 'text-red-400' : 'text-emerald-400'}`}>
                {hasActiveIssues ? 'Some systems are experiencing issues' : 'All Systems Operational'}
              </div>
          ...
          {incidents.length === 0 ? (
            ...
            <div className="space-y-4">
              {incidents.map((incident) => (
                <div key={incident.id} className="rounded-2xl border border-white/[0.07] bg-white/[0.03] p-5 relative overflow-hidden">
                  <div className={`absolute left-0 top-0 bottom-0 w-1 ${incident.status === 'RESOLVED' ? 'bg-emerald-500' : incident.status === 'INVESTIGATING' ? 'bg-red-500' : 'bg-amber-500'}`} />
