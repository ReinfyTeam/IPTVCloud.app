...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!channel || !channel.epgId) {
      return NextResponse.json({ ok: false, error: 'Channel or EPG ID not found' }, { status: 404 });
    }
    ...
    if (!res || !res.found) {
      return NextResponse.json({ ok: false, error: 'EPG data not found for ' + channel.epgId }, { status: 404 });
    }
